...
import { FileText, Upload, ShieldCheck, CheckCircle, RotateCcw, ChevronRight, XCircle, DollarSign, UploadCloud, UserCircle } from 'lucide-react';
...
export default function LoanDemo() {
  ...
  const reset = () => {
    ...
    setEventLog([]);
    setFormData({
      name: '', income: 50000, loanAmount: 10000, term: 12, purpose: 'personal', documentUploaded: false, kycAgreed: false
    });
    InsightX.trackEvent('Loan_Apply_Viewed', { userId: demoUserId });
  };
      ...
      <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
            ...
            {steps.map((label, index) => {
              ...
              return (
                <div key={label} className="flex flex-col items-center flex-1 relative">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold z-10 transition-all ${
                    completed ? 'bg-emerald-500 text-white shadow-md' : active ? 'bg-blue-600 text-white ring-4 ring-blue-100 shadow-lg' : 'bg-gray-100 text-gray-400'
                  }`}>
                    {completed ? '✓' : index + 1}
                  </div>
                  <div className={`text-xs mt-2 font-semibold ${completed ? 'text-emerald-600' : active ? 'text-blue-600' : 'text-gray-400'}`}>
                    {label}
                  </div>
                  {/* Connector Line */}
                  {index < steps.length - 1 && (
                    <div className={`absolute top-4 left-1/2 w-full h-[2px] transition-all -z-1 ${completed ? 'bg-emerald-500' : 'bg-gray-100'}`} />
                  )}
                </div>
              );
            })}
          ...
          <div className="min-h-[360px] flex flex-col justify-center">
            ...
            {currentStep === 0 && (
              <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                <h3 className="text-2xl font-bold text-gray-900 mb-6 font-display">Let's build your loan offer</h3>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <span className="text-lg font-bold text-blue-600">${formData.income.toLocaleString()}</span>
                    </div>
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if(parseInt(e.target.value) % 50000 === 0) InsightX.trackEvent('Income_Slider_Used', { userId: demoUserId, val: e.target.value });
                      }}
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-2">
                      <label className="text-sm font-semibold text-gray-700">Requested Loan Amount</label>
                      <span className="text-xl font-bold text-emerald-600">${formData.loanAmount.toLocaleString()}</span>
                    </div>
                    <input 
                      type="range" min="1000" max="100000" step="1000"
                      value={formData.loanAmount}
                      onChange={(e) => updateForm('loanAmount', parseInt(e.target.value))}
                ...
                <div className="mt-10 flex justify-end">
                  ...
                  >
                    {processing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Check Eligibility'}
            ...
            {currentStep === 1 && (
              <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                <h3 className="text-2xl font-bold text-gray-900 mb-2 font-display">Verify your income</h3>
                <p className="text-gray-500 mb-8">Please upload a recent payslip or tax return to verify your declared income of <strong>${formData.income.toLocaleString()}</strong>.</p>
                ...
                >
                  {formData.documentUploaded ? (
                    <>
                      <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mb-4"><FileText size={32} /></div>
                      <h4 className="font-bold text-emerald-900 text-lg">payslip_2024.pdf</h4>
                      <p className="text-emerald-600 text-sm mt-1">2.4 MB • Uploaded successfully</p>
                    </>
                  ) : (
                    <>
                      <div className="w-16 h-16 bg-gray-100 text-gray-500 rounded-full flex items-center justify-center mb-4"><UploadCloud size={32} /></div>
                      <h4 className="font-bold text-gray-900 text-lg">Click to browse or drag file here</h4>
                ...
                <div className="mt-10 flex justify-end">
                  ...
                  >
                    {processing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Continue'}
                    {!processing && <ChevronRight size={18} />}
                  </button>
                </div>
              </div>
            ...
            {currentStep === 2 && (
              <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                <h3 className="text-2xl font-bold text-gray-900 mb-2 font-display">Final Verification</h3>
                <p className="text-gray-500 mb-8">Review your terms and complete your identity check.</p>
                
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-200 mb-6">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-500">Loan Amount</span>
                    <span className="font-bold text-gray-900">${formData.loanAmount.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-500">Annual Percentage Rate (APR)</span>
                    <span className="font-bold text-emerald-600">6.49%</span>
                  </div>
                  <hr className="my-3 border-gray-200" />
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-gray-900">Est. Monthly Payment</span>
                    <span className="text-xl font-black text-blue-600">${Math.round(formData.loanAmount / formData.term * 1.05)}/mo</span>
                  </div>
                </div>
                  ...
                  <div className="pt-0.5">
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if(e.target.checked) InsightX.trackEvent('Terms_Consented', { userId: demoUserId });
                      }}
                    />
                  </div>
                  <span className="text-sm text-gray-600 leading-relaxed">
                    I agree to the <a href="#" className="text-blue-600 font-medium hover:underline">Terms of Service</a> and authorize a soft credit inquiry to finalize my application. This will not impact my credit score.
                  </span>
                </label>
                ...
                <div className="mt-10 flex justify-end">
                  ...
                  >
                    {processing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Sign & Submit Application'}
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="animate-in zoom-in-95 duration-500 flex flex-col items-center justify-center text-center py-8">
                {outcome === 'approved' ? (
                  <>
                    ...
                    <h3 className="text-3xl font-black text-gray-900 mb-3 tracking-tight">Approved! 🎉</h3>
                    <p className="text-gray-500 max-w-sm mb-8">
                      Congratulations {formData.name}, your ${formData.loanAmount.toLocaleString()} loan has been instantly approved based on your application.
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <h3 className="text-3xl font-black text-gray-900 mb-3 tracking-tight">Application Declined</h3>
                    <p className="text-gray-500 max-w-sm mb-8">
                      We're sorry {formData.name}, but we cannot approve a ${formData.loanAmount.toLocaleString()} loan for your stated income bracket at this time.
                    </p>
                  </>
                )}
        ...
        <div className="w-full lg:w-80 flex flex-col gap-4">
          <div className="bg-slate-900 text-slate-300 rounded-3xl p-6 border border-slate-800 shadow-xl flex-1 max-h-[600px] flex flex-col font-mono text-sm relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-700">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-white tracking-widest text-xs uppercase">SDK Stream</h4>
              </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2">
              {eventLog.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-600 space-y-3 opacity-60">
                  <ShieldCheck size={32} />
                  <span className="text-center text-xs leading-relaxed max-w-[200px]">Waiting for interaction... Interact with the UI to see captured events.</span>
                </div>
              ) : (
                eventLog.map((e, i) => (
                  <div key={i} className="animate-in fade-in slide-in-from-bottom-2 flex gap-3 group">
                    <div className="text-slate-600 text-[10px] mt-0.5 shrink-0 w-12">{e.time.split(' ')[0]}</div>
                    <div className="flex-1">
                      <div className="text-emerald-400 font-semibold mb-0.5 group-hover:text-emerald-300 transition-colors">{e.eventName}</div>
          ...
          <div className="bg-blue-50 text-blue-800 rounded-2xl p-5 border border-blue-100 text-sm leading-relaxed shadow-sm">
            ...
            Try intentionally causing an approval or rejection by adjusting the income and loan amount sliders.
          </div>
        </div>
